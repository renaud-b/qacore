...
function GetUserProfile(address) {
    ...
    const graph = new GraphElement(UserProfileGraphID, Blackhole.LoadGraph(UserProfileGraphID));
    const profileNode = graph.children().find(n => n.object["profile-address"] === address);
    if (!profileNode) {
        return { status: "error", address: address, message: "Profile not found." };
    }
    return {
        status: "ok",
        profile: profileNode.object
    };
}
...
function FinishMission(address, xp) {
    ...
    Blackhole.UpdateElement(UserProfileGraphID, profileNode.id, "profile-xp", xpTotal);
    Blackhole.UpdateElement(UserProfileGraphID, profileNode.id, "profile-level", level);
