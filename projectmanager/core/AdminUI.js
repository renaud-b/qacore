...
const AdminUI = {
    renderMissionList(status, missions) {
        const containerMap = {
    ...
    refreshMissionById(missionGraphID, id) {
        Blackhole.getGraph(missionGraphID).then((graph) => {
            ...
            // Supprimer l'ancienne version de cette mission
    ...
    renderCard(mission, status, container) {
        ...
        `;
            ...
            let actions = "";
            if (status === "submitted") {
                actions += `<button class="px-3 py-1 bg-blue-600 text-white rounded text-sm" onclick='AdminUI.validate(${JSON.stringify(mission)})'>Valider mission</button>`;
            } else {
        ...
        if (assignedUser) {
            Wormhole.getUserProfile(assignedUser).then((profileGraph) => {
                const card = buildCard(mission, status, profileGraph.object.graphName)
    ...
    validate(mission) {
        openValidationModal({
            ...
            xp: mission["mission-xp_reward"],
            comment: mission["mission-comment"] || "",
            report: {} // à charger plus tard
        });

        // Pré-remplir pour confirm button
