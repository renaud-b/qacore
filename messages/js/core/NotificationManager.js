const NotificationManager = {
    ...
    updateNotificationState: function () {
        ...
        const actions = NotificationManager._createNewNotificationFilter(MessageAPI.userAddress, graph, convID, isChecked)
        if (actions.length > 0) {
            console.log("actions: ", actions)
            const groupAction = Blackhole.Actions.makeGroup(graph.graphID, ...actions)
    ...
    _createNewNotificationFilter: (userAddress, graph, convID, isChecked) => {
        ...
        function createNotification(notifierPath, notificationNodeName) {
            ...
            actions.push(Blackhole.Actions.update(notifierID, "children", notificationNodeName))
            const expectedPath = notifierPath + "/" + notificationNodeName
            const filterActions = NotificationManager.createFilterActions(expectedPath, convID, userAddress)
        ...
        if (!graph.hasNext("dapps")) {
            actions.push(Blackhole.Actions.update(graph.object.id, "children", "dapps"))
            const expectedDappsPath = graph.object.path + "/dapps"
            actions.push(Blackhole.Actions.update(MD5(expectedDappsPath), "children", "notifier"))
            ...
            if (!dapps.hasNext("notifier")) {
                actions.push(Blackhole.Actions.update(dapps.object.id, "children", "notifier"))
                if (isChecked) {
    ...
    createFilterActions: (nodePath, convID, userAddress) => {
        ...
        actions.push(Blackhole.Actions.update(nodeID, "description", "New private message"))
        ...
        actions.push(Blackhole.Actions.update(convPrefixID, "expected-value", "urn:pi:graph:action:" + convID))
        ...
        actions.push(Blackhole.Actions.update(notID, "children", "address_is_not_me"))
        const addressIsNotMeID = MD5(notPath + "/address_is_not_me")

        actions.push(Blackhole.Actions.update(addressIsNotMeID, "type", "equals"))
        actions.push(Blackhole.Actions.update(addressIsNotMeID, "tx-property", "sender_blockchain_address"))
        actions.push(Blackhole.Actions.update(addressIsNotMeID, "expected-value", userAddress))
        return actions
    }
};
